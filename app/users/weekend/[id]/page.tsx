...
function UserProductDetailContent() {
    ...
    // Assuming this page is ONLY for weekend products now, we might force isWeekend to true or just check param for safety
    ...
    useEffect(() => {
        const fetchProduct = async () => {
            if (!id) return;
            try {
                // Since this is the weekend product page, always query WeekendProducts
                const collectionName = "WeekendProducts";
    ...
    const handlePurchase = async () => {
        ...
        try {
            await runTransaction(db, async (transaction) => {
                ...
                orderData.weekendBalance = weekendReward + product.dailyIncome; // Add bonus + first day's income
                orderData.rewardPercent = rewardRate;

                transaction.set(orderRef, orderData);

                // 5. Deduct Balance from "Recharge" field and Update Specific Income Rate
                const userUpdate: any = {
                    Recharge: rechargeBalance - product.price
                };

                // Weekend products add income rate and reward to WeekendBalance
                userUpdate.WeekendBalance = increment(weekendReward);
    ...
    const isTimedOut = (() => {
        if (!product?.endTime) return false;
        const now = new Date();
        const currentTime = now.getHours() * 60 + now.getMinutes();
        const [endH, endM] = product.endTime.split(":").map(Number);
        return currentTime > (endH * 60 + endM);
    })();
        ...
        <div className="min-h-screen bg-white text-blue-900 pb-32 overflow-hidden relative font-sans">
                ...
                <div className="flex flex-col items-center">
                    ...
                    <span className="text-[10px] font-black text-blue-900/40 tracking-widest uppercase mt-1">
                        <span className="flex items-center gap-1.5">
                            <span className="text-orange-500">ENDS IN:</span>
            ...
            <div className="fixed bottom-0 left-0 right-0 p-6 bg-white/95 backdrop-blur-md border-t border-blue-50 z-50 flex flex-col gap-4 max-w-lg mx-auto pb-10">
                <button
                    ...
                    disabled={isBuying || (statusMsg?.type === 'success') || isTimedOut}
                    className={`w-full h-16 rounded-2xl font-black text-xs tracking-[0.2em] uppercase transition-all shadow-lg active:scale-[0.98] flex items-center justify-center gap-2 ${isBuying || isTimedOut
