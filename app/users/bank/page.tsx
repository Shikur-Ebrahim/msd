            ...
            <div className="relative z-10 max-w-lg mx-auto min-h-screen flex flex-col">
                <header className="fixed top-0 left-0 right-0 max-w-lg mx-auto bg-white/90 backdrop-blur-2xl z-50 px-6 py-6 flex items-center justify-between border-b border-slate-50">
                    ...
                    <h1 className="text-xl font-black text-slate-900 leading-none tracking-tight">
                        {view === "list" ? "Withdrawal Bank" : "Link Your Bank"}
                    </h1>
                    ...
                    {view === "list" ? (
                        linkedBank ? (
                            ...
                            <div className="space-y-10 animate-in fade-in slide-in-from-bottom-6 duration-1000">
                                <div className="flex items-center justify-between px-2 pt-28">
                                    <h3 className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Active Bank Account</h3>
                                    <button
                                        onClick={() => router.push("/users/service")}
                                        className="text-[10px] font-bold text-orange-600 uppercase tracking-widest flex items-center gap-2 transition-all hover:bg-orange-50 px-4 py-2 rounded-xl"
                                    >
                                ...
                                <div className="bg-white rounded-[3.5rem] p-8 border border-slate-50 shadow-xl shadow-slate-900/5 relative overflow-hidden group/bank">
                                    ...
                                    <div className="flex items-center gap-6 mb-10 relative z-10">
                                        ...
                                        <div className="flex-1 min-w-0">
                                            <h4 className="text-xl font-black text-slate-900 tracking-tight leading-none mb-2 truncate">{linkedBank.bankName || "My Bank"}</h4>
                                            <div className="flex items-center gap-2">
                                                <div className="w-1.5 h-1.5 rounded-full bg-emerald-500"></div>
                                                <span className="text-[10px] font-bold text-emerald-600 uppercase tracking-[0.1em]">Verified Account</span>
                                            </div>
                                        </div>
                                        ...
                                        {!isEditing && (
                                            <button
                                                ...
                                                className="self-start text-[10px] font-black text-slate-300 hover:text-orange-500 transition-all uppercase tracking-widest bg-slate-50 px-4 py-2 rounded-xl border border-slate-100 shadow-sm active:scale-95"
                                            >
                                                Edit
                                            </button>
                                        )}
                                    ...
                                    <div className="space-y-6 relative z-10">
                                        <div className="bg-slate-50/50 rounded-[2rem] p-7 space-y-5 border border-slate-50 relative group/info">
                                            ...
                                            <div className="flex justify-between items-center gap-4">
                                                ...
                                                {isEditing ? (
                                                    <input
                                                        ...
                                                        className="text-sm font-black text-slate-900 tracking-widest font-mono bg-white border border-slate-100 rounded-lg px-3 py-2 outline-none focus:border-orange-500 w-1/2 text-right"
                                                    />
                                                ) : (
                                            ...
                                            <div className="flex justify-between items-center gap-4 border-t border-slate-100 pt-4">
                                                <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Holder Name</span>
                                                {isEditing ? (
                                                    <input
                                                        type="text"
                                                        value={editForm.holderName}
                                                        onChange={(e) => setEditForm({ ...editForm, holderName: e.target.value })}
                                                        className="text-xs font-black text-slate-800 bg-white border border-slate-100 rounded-lg px-3 py-2 outline-none focus:border-orange-500 w-1/2 text-right"
                                                    />
                                                ) : (
                                            ...
                                            {isEditing && (
                                                <div className="flex gap-3 pt-2">
                                                    <button
                                                        ...
                                                        disabled={updating}
                                                        className="flex-1 py-3 bg-slate-100 text-slate-400 rounded-xl text-[10px] font-bold uppercase tracking-widest hover:bg-slate-200 transition-all disabled:opacity-50"
                                                    >
                                                        Cancel
                                                    </button>
                                                    <button
                                                        ...
                                                        disabled={updating}
                                                        className="flex-2 py-3 bg-slate-900 text-white rounded-xl text-[10px] font-bold uppercase tracking-widest hover:bg-black transition-all flex items-center justify-center gap-2 px-8 disabled:opacity-50"
                                                    >
                                                        {updating ? (
                            ...
                            <div className="h-full flex flex-col items-center justify-center pt-32 pb-16 text-center space-y-12 animate-in zoom-in-95 duration-1000">
                                ...
                                <div className="space-y-3 max-w-xs mx-auto">
                                    <h2 className="text-2xl font-black tracking-tight text-slate-900">Add Bank Detail</h2>
                                    <p className="text-[11px] font-bold text-slate-400 uppercase tracking-[0.15em] leading-relaxed px-4">Link your preferred bank account to receive withdrawals instantly.</p>
                                </div>

                                <button
                                    onClick={() => setView("connect")}
                                    className="w-full py-6 bg-slate-900 text-white rounded-[2rem] text-[10px] font-black uppercase tracking-[0.3em] shadow-2xl shadow-slate-900/20 active:scale-95 transition-all flex items-center justify-center gap-4 hover:bg-black"
                                >
                        ...
                        <div className="h-full flex flex-col items-center justify-center py-32 text-center space-y-10 animate-in zoom-in-95 duration-700">
                            ...
                            <div className="space-y-4 max-w-xs mx-auto px-4">
                                <h3 className="text-2xl font-black text-slate-900 uppercase tracking-tight">Account Linked</h3>
                                <p className="text-[11px] font-bold text-slate-400 uppercase tracking-[0.1em] leading-relaxed">
                                    A bank account is already linked. To protect your funds, changing account details requires verification from our support team.
                                </p>
                            </div>

                            <button
                                onClick={() => router.push("/users/service")}
                                className="w-full py-6 bg-orange-500 text-white rounded-[2rem] text-[10px] font-black uppercase tracking-[0.3em] shadow-xl shadow-orange-500/20 active:scale-95 transition-all"
                            >
                                Contact Support
                            </button>
                        </div>
                        ...
                        <div className="space-y-12 animate-in fade-in slide-in-from-top-6 duration-1000 max-w-sm mx-auto w-full pt-32 pb-44">
                            <div className="text-center space-y-3">
                                ...
                                <div className="space-y-1">
                                    <h2 className="text-2xl font-black text-slate-900">Secure Setup</h2>
                                    <p className="text-[10px] font-bold text-slate-400 uppercase tracking-[0.3em]">Bank Info Registration</p>
                                </div>
                            </div>
                            ...
                            <form onSubmit={handleConnect} className="space-y-8">
                                <div className="space-y-6">
                                    ...
                                    <div className="space-y-3 relative">
                                        <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest ml-4">Financial Institution</label>
                                        <div
                                    ...
                                    <div className="space-y-3">
                                        <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest ml-4">Account Holder Name</label>
                                        <div className="relative group">
                                    ...
                                    {/* Account Number */}
                                    <div className="space-y-3">
                                        <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest ml-4">Account Number</label>
                                        <div className="relative group">
                                            <div className="absolute left-6 top-1/2 -translate-y-1/2 w-10 h-10 rounded-xl bg-slate-50 flex items-center justify-center text-slate-200">
                                                <Hash size={20} />
                                            </div>
                                            <input
                                                ...
                                                className="w-full h-20 pl-16 pr-8 rounded-[1.8rem] bg-white border border-slate-50 focus:border-slate-900 transition-all font-bold text-sm text-slate-900 outline-none font-mono tracking-widest"
                                            />
                                        </div>
                                    </div>
                                </div>
                                ...
                                <button
                                    type="submit"
                                    disabled={submitting}
                                    className="w-full py-6 bg-slate-900 text-white rounded-[2rem] text-[10px] font-black uppercase tracking-[0.3em] flex items-center justify-center gap-4 shadow-xl shadow-slate-900/20 active:scale-95 disabled:opacity-50 mt-10 transition-all hover:bg-black"
                                >
                                    {submitting ? (
                ...
                <footer className="p-10 text-center relative z-10 mt-auto">
                    <div className="inline-flex items-center gap-3 px-5 py-2.5 rounded-xl bg-slate-50 border border-slate-100 italic">
                        ...
                        <span className="text-[10px] font-bold text-slate-400 tracking-wider">Secured Network</span>
                    </div>
                </footer>
            </div>
        </div>
